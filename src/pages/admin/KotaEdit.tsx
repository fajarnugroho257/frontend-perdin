...
import { detailDataKota, updateKota, getAllPulau, getAllProvinsi } from "../../api/auth";
...
const KotaEdit = () => {
    ...
    useEffect(() => {
        const getDetailData = async () => {
            try {
                const response = await detailDataKota(id ?? '0');
                if (response.success) {
                    allProvinsi(response.data.pulau_id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // update
        try {
            const response = await updateKota(formData, id ?? '0');
            if (response.success) {
                Swal.fire({
                    title: "Succes!",
                    text: response.message,
