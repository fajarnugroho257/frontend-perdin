...
import { allDataKota, getAllPref, storePerdin } from "../../api/auth";
...
const Perdin = () => {
    ...
    const getAllPreference = async () => {
        try {
            const response = await getAllPref();
            // console.log(response.data);
            setDataPreference(response.data);
        } catch (error) {
            console.error("Gagal mengambil data pref:", error);
        }
    }
    // jumlah biaya
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // store
        try {
            const response = await storePerdin(formData);
            if (response.success) {
                Swal.fire({
                    title: "Succes!",
